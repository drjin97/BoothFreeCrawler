        ...
        for url_idx, (url, subfolders) in enumerate(zip(self.urls, self.subfolders_list), 1):
            try:
                ...
                for subfolder in subfolders:
                    ...
                    filename = content_disposition.split('filename=')[-1].strip('"') if 'filename=' in content_disposition else f'{item_id}.zip'
...
class SubfolderDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.current_folders_list.setMaximumHeight(100)  # 높이 제한
        ...
        self.recent_folders_list.setMaximumHeight(100)  # 높이 제한
        self.recent_folders_list.itemDoubleClicked.connect(self.add_recent_folder)
        self.update_recent_folders_list()  # 초기 최근 폴더 목록 표시
        ...
        self.folder_list.setMaximumHeight(100)  # 높이 제한
...
class BoothDownloader(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # URL 입력 영역
        url_scroll = QScrollArea()
        url_scroll.setWidgetResizable(True)
        url_scroll.setMinimumHeight(200)
        ...
        url_scroll.setWidget(url_container)
        layout.addWidget(url_scroll)
